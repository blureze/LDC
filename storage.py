...
collection_name = "plan_a_conversation"	# name of the table

client = MongoClient(url)
db = client[db_name]	# database
collection = db[collection_name]	# table

def post(speaker, dialog_id, content, emotion, order):
...
with open("test.txt", 'r', encoding='utf-8') as f:
	...
	for line in f:
		...
		speaker = tmp[0]
		content = tmp[1]
		dialog_id = "test1"	# filename
		emotion = "sadness"
		order += 1

		post(speaker, dialog_id, content, emotion, order)
f.closed 
